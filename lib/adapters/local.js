...
class LocalAdapter extends Adapter {
  ...
  async write (filePath, contents, options = {}) {
    await this.ensureDirectory(path.dirname(this.applyPrefix(filePath)))
    return super.write(filePath, contents, options)
  }
  ...
  async writeStream (filePath, readStream, options = {}) {
    ...
    let destStream = this.fs.createWriteStream(filePath)

    // re-dispatch any error on the write stream back to readable stream.
    destStream.once('error', err => readStream.emit('error', err))
